...
public class LivingEntity extends Entity implements Damageable {
    ...
    public float getHealth(){return living.getHealth();} public void setHealth(double h){living.setHealth((float)Math.max(0,Math.min(getMaxHealth(),h)));} public float getMaxHealth(){return living.getMaxHealth();} public int getNoDamageTicks(){return living.invulnerableTime;} public void setNoDamageTicks(int ticks){living.invulnerableTime=Math.max(0,ticks);} public double getEyeHeight(){return living.getEyeHeight();}
    ...
    public double getAttribute(Attribute attribute){return living.getAttributeValue(attribute.nms());} public void addPotionEffect(PotionEffect effect){living.addEffect(effect.toNms());}
